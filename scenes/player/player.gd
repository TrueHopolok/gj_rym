...
func _physics_process(delta: float) -> void:
	...
	_movement_vertical()
	_update_state()
	if OS.is_debug_build(): _debug_state_label.text = PLAYER_STATES_TO_STRING[state]
	move_and_slide()
...
func _movement_horizontal(delta: float) -> void:
	var input_dir: float = Input.get_axis(&"left", &"right")
	if input_dir == 0: return
	if input_dir != sign(velocity.x): velocity.x = 0.0
...
func _resistance_vertical(delta: float) -> void:
	...
	if velocity.y < 0.0 && Input.is_action_just_released(&"jump"): velocity.y *= 0.4
	if velocity.y > 0.0: velocity.y = minf(velocity.y + delta * get_gravity().y, MAX_FALLING_SPEED)


func _movement_vertical() -> void:
	if (_buffer_jump.is_stopped()): return
	if (!is_on_floor() && _buffer_coyote.is_stopped()): return
	_buffer_jump.stop()
	_buffer_coyote.stop()
...
func _update_state() -> void:
	if (!is_on_floor()): state = PLAYER_STATES.JUMP
	elif abs(velocity.x): state = PLAYER_STATES.MOVE
